...
def rotate_image_test(image, angle):
    ...
    new_width = int(abs(height * math.sin(angle)) +
                    abs(width * math.cos(angle)))
    new_height = int(abs(height * math.cos(angle)) +
    ...
    for x in range(new_width):
        for y in range(new_height):
            old_x = int(math.cos(
                angle) * (x - center[0]) + math.sin(angle) * (y - center[1]) + width // 2)
            old_y = int(-math.sin(angle) * (x -
                        center[0]) + math.cos(angle) * (y - center[1]) + height // 2)
...
resized1000x1000 = bl_resize(image, 1000, 1000)

cv2.imwrite("resized1000x1000.png", resized1000x1000)
